...
class Paginator(object):
    ...
    works. ``limit`` & ``offset`` are used in place of ``page``
    ...
    Forked from django-testypie with few modifications.
    
    - ``skip_count`` - Speed requests by avoiding constly count() operations
    
    """
    def __init__(self, request_data, objects, resource_uri=None, limit=None, offset=0, max_limit=200, skip_count=False, collection_name='objects'):
        """
    ...
    def get_limit(self):
        ...
        try:
            ...
            raise ApiBadRequestException("Invalid limit '%s' provided. Please provide a positive integer." % limit)

        if limit < 0:
            raise ApiBadRequestException("Invalid limit '%s' provided. Please provide a positive integer >= 0." % limit)
    ...
    def get_offset(self):
        ...
        try:
            ...
            raise ApiBadRequestException("Invalid offset '%s' provided. Please provide an integer." % offset)

        if offset < 0:
            raise ApiBadRequestException("Invalid offset '%s' provided. Please provide a positive integer >= 0." % offset)
    ...
    def get_next(self, limit, offset, count):
        ...
        page. If not available, this returns ``None``.
        """
        if self.skip_count == False and offset + limit >= count:
            return None
